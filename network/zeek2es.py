...
def parseargs():
    parser = MyParser(
        description="Process Zeek ASCII logs into ElasticSearch.",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "filename", help="The Zeek log in *.log or *.gz format.  Include the full path."
    )
    parser.add_argument(
        "-i", "--esindex", help="The Elasticsearch index/data stream name."
    )
    parser.add_argument(
        "-u",
        "--esurl",
        default="http://localhost:9200/",
        help="The Elasticsearch URL.  Use ending slash.  Use https for Elastic v8+. (default: http://localhost:9200/)",
    )
    parser.add_argument(
        "--user", default="", help="The Elasticsearch user. (default: disabled)"
    )
    parser.add_argument(
        "--passwd",
        default="",
        help="The Elasticsearch password. Note this will put your password in this shell history file.  (default: disabled)",
    )
    parser.add_argument(
        "-l",
        "--lines",
        default=10000,
        type=int,
        help="Lines to buffer for RESTful operations. (default: 10,000)",
    )
    parser.add_argument(
        "-n",
        "--name",
        default="",
        help="The name of the system to add to the index for uniqueness. (default: empty string)",
    )
    parser.add_argument(
        "-k",
        "--keywords",
        nargs="+",
        default="service",
        help="A list of text fields to add a keyword subfield. (default: service)",
    )
    parser.add_argument(
        "-a",
        "--lambdafilter",
        default="",
        help="A Python lambda function, when eval'd will filter your output JSON dict. (default: empty string)",
    )
    parser.add_argument(
        "-f",
        "--filterfile",
        default="",
        help="A Python function file, when eval'd will filter your output JSON dict. (default: empty string)",
    )
    parser.add_argument(
        "-y",
        "--outputfields",
        nargs="+",
        default="",
        help="A list of fields to keep for the output.  Must include ts. (default: empty string)",
    )
    parser.add_argument(
        "-d",
        "--datastream",
        default=0,
        type=int,
        help="Instead of an index, use a data stream that will rollover at this many GB.\nRecommended is 50 or less.  (default: 0 - disabled)",
    )
    parser.add_argument(
        "-o",
        "--logkey",
    ...
    parser.add_argument(
        ...
        metavar=("fieldname", "filename"),
        default="",
        help="A field to filter with keys from a file.  Example: uid uid.txt.  (default: empty string - disabled)",
    )
    parser.add_argument(
        "-g",
        "--ingestion",
        action="store_true",
        help="Use the ingestion pipeline to do things like geolocate IPs and split services.  Takes longer, but worth it.",
    )
    parser.add_argument(
        "-p",
        "--splitfields",
        nargs="+",
        default="",
        help="A list of additional fields to split with the ingestion pipeline, if enabled.\n(default: empty string - disabled)",
    )
    parser.add_argument(
        "-j", "--jsonlogs", action="store_true", help="Assume input logs are JSON."
    )
    parser.add_argument(
        "-r",
        "--origtime",
        action="store_true",
        help="Keep the numerical time format, not milliseconds as ES needs.",
    )
    parser.add_argument(
        "-t",
        "--timestamp",
        action="store_true",
        help="Keep the time in timestamp format.",
    )
    parser.add_argument(
        "-s",
        "--stdout",
        action="store_true",
        help="Print JSON to stdout instead of sending to Elasticsearch directly.",
    )
    parser.add_argument(
        "-b",
        "--nobulk",
        action="store_true",
        help="Remove the ES bulk JSON header.  Requires --stdout.",
    )
    parser.add_argument(
        "-c",
        "--cython",
        action="store_true",
        help="Use Cython execution by loading the local zeek2es.so file through an import.\nRun python setup.py build_ext --inplace first to make your zeek2es.so file!",
    )
    parser.add_argument(
        "-w",
        "--hashdates",
        action="store_true",
        help="Use hashes instead of dates for the index name.",
    )
    parser.add_argument(
        "-z",
        "--supresswarnings",
        action="store_true",
        help="Supress any type of warning.  Die stoically and silently.",
    )
...
def senddatastream(args, es_index, mappings):
        ...
        "policy": {
            "phases": {
                "hot": {
                    "actions": {
                        "rollover": {
                            "max_primary_shard_size": "{}GB".format(args["datastream"])
                        }
                    }
                }
            }
    ...
    requests.put(
        args["esurl"] + "_ilm/policy/zeek-lifecycle-policy",
        headers={"Content-Type": "application/json"},
    ...
    index_template = {
        ...
        "priority": 500,
        "template": {
            "settings": {"index.lifecycle.name": "zeek-lifecycle-policy"},
            "mappings": mappings["mappings"],
        },
    }
    requests.put(
        args["esurl"] + "_index_template/" + es_index,
        headers={"Content-Type": "application/json"},
...
def sendpipeline(args, ingest_pipeline):
    ...
    requests.put(
        args["esurl"] + "_ingest/pipeline/zeekgeoip",
        headers={"Content-Type": "application/json"},
...
def main(**args):
    ...
    if not args["jsonlogs"]:
        ...
        zcat_process = subprocess.Popen(zcat_name + [filename], stdout=subprocess.PIPE)

        head_process = subprocess.Popen(
            ["head"], stdin=zcat_process.stdout, stdout=subprocess.PIPE
        )

        grep_process = subprocess.Popen(
            ["grep", "#open"], stdin=head_process.stdout, stdout=subprocess.PIPE
        )
        ...
        grep_process = subprocess.Popen(
            ["grep", "#path"], stdin=head_process.stdout, stdout=subprocess.PIPE
        )
        ...
        grep_process = subprocess.Popen(
            ["grep", "#fields"], stdin=head_process.stdout, stdout=subprocess.PIPE
        )
        ...
        grep_process = subprocess.Popen(
            ["grep", "#types"], stdin=head_process.stdout, stdout=subprocess.PIPE
        )
        ...
        if len(types) > 0 and len(fields) > 0:
            read_tsv = csv.reader(
                io.TextIOWrapper(grep_process.stdout),
                delimiter="\t",
            ...
            for i in range(len(fields)):
                if types[i] == "time":
                    ...
                    if fields[i] in keywords:
                        mappings["mappings"]["properties"][fields[i]] = {
                            "type": "text",
            ...
            for row in read_tsv:
                ...
                if (
                    ...
                    and "ts" in d
                    and (
                        not filterkeys_field
                        or (filterkeys_field and d[filterkeys_field] in filterkeys)
                    )
                ):
                    ...
                    if not filter_data:
                        ...
                        if not args["stdout"]:
                            ...
                            if (
                                not putpipeline
                                and len(ingest_pipeline["processors"]) > 0
                            ):
        ...
        while True:
            ...
            if "ts" in j_data:
                ...
                if not filterkeys_field or (
                    filterkeys_field and j_data[filterkeys_field] in filterkeys
                ):
