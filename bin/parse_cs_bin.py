...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--show-passwords",
        action="store_true",
        help="Show password from database.",
    )
    ...
    for filename in files:
        try:
            ...
            output_folder = path.join(
                args.prefix,
                f"{path.dirname(filename).replace('./', '').replace('/', '_')}",
            )
            ...
            if datatype in [
                ...
                with open(
                    path.join(output_folder, f"{datatype}.csv"), "w", newline=""
                ) as csvfile:
                    ...
                    if datatype in ["archives"]:
                        ...
                        for entry in tempdata[0]:
                            bindata.append(
                                {
                                    entry.annotations[1]: entry.annotations[2],
                                    entry.annotations[3]: entry.annotations[4],
                                    entry.annotations[5]: entry.annotations[6],
                                }
                            )
                    # Output data
                    if len(bindata) > 0:
        ...
        with open(path.join(folder, "c2info.csv"), newline="") as c2file:
            ...
            with open(path.join(folder, "sessions.csv"), newline="") as session_file:
                ...
                with open(path.join(folder, "c2usage.csv"), "w", newline="") as csvfile:
                    ...
                    if len(usage_list) > 0:
                        writer = csv.DictWriter(
                            csvfile, fieldnames=usage_list[0].keys()
                        )
